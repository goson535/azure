...
class soft_slots extends WP_Widget {

	function __construct() {
		parent::__construct(
			'softs',
			__( 'SLOTS SOFT WIDGET', 'jgambling' )
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $terms ):
			...
			foreach ( $terms as $term ) {
				...
				echo '<tr>
                        <td><a href="' . get_term_link( $term_id, 'software' ) . '">' . $term->name . '</a></td>';
				if ( $term_mini_img_src ) {
					echo '<td><img class="lazy" data-src="' . $term_mini_img_src . '"></td>';
				}else{
				    echo '<td></td>';
                }
				echo '</tr>';
			...
			echo __( 'No slots soft found', 'jgambling' );
		endif;
...
function register_new_widgetslot() {
	register_widget( 'soft_slots' );
}

add_action( 'widgets_init', 'register_new_widgetslot' );
wp_reset_query();